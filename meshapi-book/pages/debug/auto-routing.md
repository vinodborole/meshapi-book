...
you. Most confusion is about *which*model ran,

*what*you were billed, and

*where*it’s supported. See

[Auto Routing](/docs/capabilities/auto-routing)for the full guide.
...
`model: "auto"` on any other endpoint (images, video, audio) won’t
route — use a concrete model ID there.
## Which model actually served my request?

Mesh injects the resolved model into the response, but
**where**depends on streaming:
...
With Auto Routing you’re billed for
**two**things: the tokens of the resolved model

**plus**the tokens consumed by the internal classifier model. Both show up in the usage dashboard. For cost-sensitive or high-volume paths, pin a specific model instead.
...
The classification step adds an extra internal LLM call before your request is forwarded, which increases time-to-first-token. For latency-sensitive interactive use, pin a model rather than using`auto`.
## Still stuck?
