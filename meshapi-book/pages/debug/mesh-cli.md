---
type: Web Page
title: Mesh CLI - Mesh API
description: Fix Mesh CLI install, sign-in, and runtime problems.
resource: https://developers.meshapi.ai/debug/mesh-cli
timestamp: '2026-08-10T07:50:31.317333+00:00'
---

[Install the Mesh CLI](/docs/cli/install)and

[Using the Mesh CLI](/docs/cli/usage). If something isn’t working, find your symptom below.

## ‘meshapi: command not found’ / ‘not recognized’

The install worked, but your terminal hasn’t picked up the new PATH — almost always because the
**close-and-reopen**step was skipped.
...
- Redo [Install → Step 1](/docs/cli/install) .
...
Re-enter your API key from inside the CLI — no need to restart:`rsk_...` key. Grab a fresh one from
...
`spend_limit_exceeded` / `402` you’d see from the API — see
...
`meshapi --version`. See
[Staying up to date](/docs/cli/commands#staying-up-to-date)for details.

## It won’t write files or run commands automatically

That’s the
**permission mode**, shown in the colored bar at the bottom.
...
- Even in **bypass permissions** , genuinely dangerous actions (`rm -rf` ,`sudo` , writing to`~/.ssh` ) still stop and ask. See[Commands & permission modes](/docs/cli/commands#permission-modes) .

## What do the y / a / n prompts mean?

When the CLI wants to touch your computer it asks to
**approve tool call**:
...
`a` and want prompts back, restart the CLI or switch to a stricter permission mode.
...
- Prefer not to choose? `/route auto` lets Mesh pick per prompt (see[Auto Routing](/docs/capabilities/auto-routing) ).

## Pasting the key or text doesn’t work (Windows)

In PowerShell, paste with
**right-click**, not

`⌘/Ctrl+V`. When pasting your
API key on first launch, **nothing shows on screen**— it’s hidden for security. Type your input and press Enter.
...
Email
**with the command you ran, the full output, and your CLI version (**

[contact@meshapi.ai](mailto:contact@meshapi.ai)`meshapi --version`). More options on the [Support](/docs/reference/support)page.
