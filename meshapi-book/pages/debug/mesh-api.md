...
`req_...` ID returned in the response headers and
visible in Dashboard → **Logs**. Include it in any support message — it lets us trace the exact call.
## First-response checklist

Before digging in, confirm the basics:
1. **Base URL** — requests go to`https://api.meshapi.ai` .
2. **Auth header** —`Authorization: Bearer rsk_...` (note the`Bearer` prefix and the`rsk_` key prefix).
3. **Key is active** — check Dashboard →**API Keys** ; a deactivated key returns`403` .
4. **Balance** — a zero balance or hit spend cap returns`402` . Check Dashboard →**Billing** .
5. **Model exists** — confirm the model ID is enabled via`GET /v1/models` .
6. **Read `error.code`** — the response body’s`error.code` field names the exact failure.
...
Every failed request surfaces in
**two**places that share one
...
`x-request-id` response header, so you can
log it even before parsing the body.
**2. What you see in Dashboard → Logs**— the matching row at

[app.meshapi.ai](https://app.meshapi.ai)→

**Logs**:

**Status: error**to list only failed calls, then open a row for its full error code and message. The

`req_...` ID is the link between what your
...
`Retry-After` header when present. All official SDKs retry `429`
...
- Some models require [BYOK](/docs/capabilities/byok) or must be enabled first.
...
Not every model accepts every sampling parameter, and the provider — not Mesh — enforces this. A rejected parameter surfaces as an`upstream_error` (`500`) or a
...
- **Some models reject `temperature` and `top_p` sent together** — pick one sampling strategy, not both.
- Out-of-range values (`temperature` outside`0–2` ,`top_p` outside`0–1` ) are also rejected.

`temperature` **or**

`top_p`, and omit both for
reasoning models.
...
- If one model fails repeatedly, try [Auto Routing](/docs/capabilities/auto-routing) or a different model.
...
Streams do
**not**auto-retry. A dropped connection raises an error with
...
**Never call Mesh API from client-side code.**Route requests through your own backend proxy that holds the key. See

[Authentication → Security Best Practices](/docs/getting-started/authentication).

### Responses are slow or latency is high

- First token latency depends on the model and provider load — try a faster model or [Auto Routing](/docs/capabilities/auto-routing) .
- Use **streaming** so users see tokens as they generate instead of waiting for the full response.
- For bulk, non-interactive work use the [Batch API](/docs/capabilities/batch-api) — cheaper and built for throughput.
- Large RAG contexts add latency; trim retrieved chunks. See [RAG](/docs/capabilities/rag) .
...
[contact@meshapi.ai](mailto:contact@meshapi.ai)
1. The **request ID** (`req_...` ) from the response headers or Dashboard logs.
2. The **model** and**endpoint** you called.
3. The **error code and message** you received.
4. The **account email** tied to your API key.

[Support](/docs/reference/support)page.
