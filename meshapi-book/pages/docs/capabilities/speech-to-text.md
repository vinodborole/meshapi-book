...
description: Text-to-speech, speech-to-text, voice management, and real-time streaming
  audio APIs.
resource: https://developers.meshapi.ai/docs/capabilities/speech-to-text
...
If the selected model doesn’t support translation, the API returns a 

`422` error. Check `GET /v1/models` to confirm a model’s capabilities.
...
The wire protocol is selected automatically from the model you pass. Standard streaming models (e.g. `openai/whisper-large-v3`) use an OpenAI-compatible frame protocol. ElevenLabs models (`elevenlabs/scribe_v2_realtime`) use ElevenLabs’ Scribe v2 realtime frames — see the [ElevenLabs models](#elevenlabs-models)subsection below.
...
**Client → server (JSON frames)**Send

`input_audio_chunk` frames with base64-encoded audio. This is the only message type the server forwards upstream — any other message type is silently dropped.
`"commit": true` to trigger a VAD commit when using `commit_strategy: manual`.
**Server → client (JSON frames)**
...
large, so **filter by**to narrow it to the voices you can actually use with the model you plan to call.

`brand` or `model`
Each voice is returned with 

`voice_id`, `name`, `brand`, `provider`, `model`,
...
`voice_id` as the `voice` field on `POST /v1/audio/speech` (or the
...
WebSocket sessions send a JSON 

`error` frame and then close with code `1000` before disconnecting.
## Available models

Together STT models support 

**batch**transcription (upload a file). Diarization, audio-event tagging, and`source_url`/`cloud_storage_url` are ElevenLabs-only features.
For Indian-language audio, the 

`language_code` parameter significantly improves accuracy. Supported codes include `hi`, `ta`, `te`, `kn`, `ml`, `mr`, `gu`, `bn`.

# Citations

1. Source page: https://developers.meshapi.ai/docs/capabilities/speech-to-text
