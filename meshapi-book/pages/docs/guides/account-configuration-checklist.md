...
- 
**Skip model allow-lists unless you truly need one.**- The `allowed_models`field restricts a key to a fixed list of models. It’s easy to get wrong:- Web search runs as `perplexity/sonar`. Leave that off the list and`POST /v1/web/search`returns`403`.
...
- 
**Batch anything that isn’t urgent.**- Batch jobs run at cheaper batch pricing where the model supports it. See [Batch API](/docs/guides/batch-api).
...
- **Where:**Dashboard → Billing (balance and spend history) and Dashboard → Logs (per-request detail). API:- `GET /v1/balance`(live balance),- `POST /v1/usage`(aggregates),- `GET /v1/usage/spend-trend`,- `GET /v1/usage/events/export`(CSV for finance), plus org-level- `GET /v1/orgs/{id}/usage`and- `/spend`.
...
- **Where:**API:- `POST /v1/orgs/{id}/invitations`.
...
- 
**Leave fallback on**(`allow_fallback: true`) — see section 3.- **Default if not set:**on.
...
- **No spend cap means unlimited spend.**Set- `spend_cap_usd`on every key.
- `allowed_models`quietly breaks web search and- `model="auto"`
- **A non-zero**No error — you just never see- `temperature`or any- `tools`skips the free cache.- `X-Cache: HIT`.
- **Templates never win.**The request and the key defaults both override a template’s model and params.
- **Your balance can dip below zero**on the last request, and a spend cap can go over by one request. Both are by design — you’re billed after each call completes.
- **Deleting a key suspends it**(- `DELETE /v1/keys/{id}`) — support can restore it. It is not gone forever.
