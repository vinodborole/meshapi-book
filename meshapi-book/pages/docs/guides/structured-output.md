...
`https://app.meshapi.ai/org/<your-org-id>/models`) — or the `supports_structured_output` flag returned by `GET /v1/models` — and switch to one with first-class support such as OpenAI models or Google Gemini (e.g. `google/gemini-2.5-flash`). See [Not getting JSON back?](/docs/guides/structured-output#not-getting-json-back)and

[How it works across providers](/docs/guides/structured-output#how-it-works-across-providers).
